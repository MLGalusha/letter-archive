...
function printHelp(): void {
  console.log(`Usage: npm run logs:query -- [options]
...
function main(): void {
  ...
  const sinceTime = Date.now() - (options.hours * 60 * 60 * 1000);
  ...
  if (entries.length === 0) {
    console.error(`No matching log entries in ${options.logDir}`);
    process.exitCode = 1;
    return;
  }

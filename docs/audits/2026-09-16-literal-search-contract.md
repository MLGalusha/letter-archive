...
Word boundaries also belong to PostgreSQL: superscript numerals are separators, while Roman numerals differ between libc and ICU regular expressions. The existing selected-row query now returns an internal word map for only names/locations and fuzzy-eligible query terms. It tokenizes the same normalized, page-marker-sanitized source as SQL admission. No transcripts are tokenized, no query is added, and no token data is exposed in the public API. Literal-only and short queries skip the map. Preview matching uses these words directly for padded trigram sets and locates repeated words sequentially in the normalized source. Real `Molly²` and `MollyⅣ` fixtures verify admission, the correct Sender/Recipient explanation, and per-word score parity in both providers, including metadata containing generated page markers.

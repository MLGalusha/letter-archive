...
A targeted word-class probe also found that Unicode `Number` includes superscript numerals that PostgreSQL treats as separators. Preview tokenization and trigram generation now use letters plus decimal digits. The real `Molly²`/`Mollly` fixture verifies SQL admission, the matching Sender preview, and direct trigram-score equality under both libc and ICU.

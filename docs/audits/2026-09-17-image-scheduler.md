...
Two real browser findings were fixed during review: WebKit needed a fresh off-DOM Image object for same-URL retry; and visible failed image elements needed replacement after background success even if their src string stayed unchanged. The reader/preloaded case receives the same bounded DOM recovery. Unit regressions preserve those boundaries, source revisits, and fitted-resolution/zoom behavior. All 28 final focused frontend tests passed (24 in the initial candidate), alongside the full frontend suite (1,201 tests), build and lint baseline (102 existing diagnostics, no increases). [Browser observations](evidence/2026-09-17-image-retry-browsers.json) record actual request counts and decoded-image state.

A final review found that lower-resolution placeholders also need their own visible-layer recovery after background success. The visible tier now owns retry/error/load callbacks; hidden-main events cannot cancel its retry. Distinct layer keys prevent React from reusing the failed element. Additional Chromium/WebKit probes supplied already-loaded thumb/mid hook state, served a real image 503 then 200 at the same URL, and injected a hidden-main load event while the visible retry was pending. In both engines, each visible placeholder was replaced, decoded successfully (naturalWidth 100), and cleared the error overlay after exactly two requests. These controlled state probes supplement the earlier end-to-end loader checks.

All three DOM retry consumers were checked for immediate failure coverage. Card and progressive images already cover the current failure; the reader now also hides its failed ready image and exposes a status during backoff/exhaustion. Panel/lightbox tests cover both delays, the final cap and successful recovery. Additional Chromium/WebKit checks supplied ready-tier hook state and real 503→200 responses: the failed reader was hidden with status before retry, then replaced by a decoded visible image after exactly two requests to the same width URL.

A final endpoint-consumer audit also covered resized images outside those wrappers. A small `RetryingImage` preserves a bare `img` node, forwarded ref, classes, caller styles, handlers and lazy-loading attributes while sharing the existing two-retry policy. It now covers all five public transcript/extra-content thumbnails, both admin home-content preview branches, both journal database-picker levels, reader initial thumbnail/minimap, and the Line Review minimap. Original images (including Line Review's main scan), blog images, upload/blob previews and speculative preload calls remain outside this transform-recovery path. No admin editing/selection behavior changed.

The expanded targeted suite passed 82 tests; independent review reran the 11 primitive/journal/reader cases. Journal tests recover both thumbnail sizes and verify the selected original URL is still inserted. In Chromium and WebKit, a fixture of 40 visible actual primitive instances recovered synthetic 503→200 responses with 80 requests total, all at the original 200/300px URLs. Permanent 503s stopped at three requests per image; replacing sources and unmounting before backoff completed left one request per source and no late retries. This models overload responses and recovery, not a production scheduler capacity or sustained-load guarantee. Existing scheduler/route tests separately exercise admission, saturation and capacity release.

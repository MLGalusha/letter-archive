...
An independent review using the separately installed WebKit runner selected the 1600px fallback for both single and multiple desktop cards. This differs from the retained Playwright CLI WebKit run above, which resolved auto sizing to 1200px/480px. Both results are valid observations of those runners; they do not establish universal Safari support for auto sizing. The fallback intentionally favors sufficient resolution over the smallest transfer. The older/fallback path therefore has smaller savings than native layout sizing and remains bounded at 1600px. Do not present the CLI candidate selections as predictions for every Safari installation.

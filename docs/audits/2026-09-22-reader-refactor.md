...
PR #217 review found two keyboard regressions: arrow navigation left focus on the old slide, and the mouse-drag suppression flag also blocked Enter/Space. Both reproduced in Chromium and WebKit (4 failing checks before the fix). Arrow navigation now moves focus with selection; only pointer-generated clicks consume drag suppression. All 4 regression checks pass.

The first CI run also identified stale fixed-body assertions and two readiness assumptions. Mobile checks now exercise document alignment and reading-position restoration instead of requiring the removed fixed-body implementation. Header hit-testing waits for interactive thumbnails after the shared transition; repeated viewport fit checks explicitly reset the preserved entry zoom. Targeted validation: 6 mobile history/scroll checks and 22 header/viewport checks passed. Raster aspect checks passed 10 repeated WebKit runs.

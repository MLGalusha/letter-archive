...
The shared strip now leaves touch momentum entirely to the browser. Pointer cancellation during native scrolling does not trigger programmatic settling. Selection follows the centered page after scrolling ends, with a contact-aware idle fallback. Explicit taps and keyboard selection retain a short centering animation; mouse dragging retains direct tracking and a short nearest-center settle without simulated momentum. Main-image progress still drives fractional strip movement when the strip is not being browsed. Width changes recalculate positions.

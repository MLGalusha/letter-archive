...
The shared strip owns horizontal pointer dragging; native vertical page scrolling and keyboard navigation remain available. Release velocity is bounded, coasting decays with additional damping near page centers, and a short settle centers the final page. Grabbing cancels the running animation and temporarily owns the strip even if main-image settlement is still finishing. Vertical wheel input does not enter horizontal browsing mode; outward wheel input at an endpoint has an idle cleanup, and explicit keyboard selection takes ownership. The main image changes once strip browsing settles. Trackpad/wheel scrolling retains native snapping. Width changes recalculate positions; an initial ResizeObserver notification no longer cancels animation when width has not changed.

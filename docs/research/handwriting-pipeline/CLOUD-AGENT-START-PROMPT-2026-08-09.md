...
research from the cloud handoff branch.
...
`handwriting-cloud-handoff`. Then completely read these files in order:
...
Before experimenting, run `python3 scripts/validate-handwriting-cloud-seed.py` and
require all tracked acting-safe sources and current page-012 records to match their

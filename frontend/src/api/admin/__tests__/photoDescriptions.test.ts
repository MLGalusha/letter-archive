...
describe('admin photo descriptions api', () => {
  it('sends AI context to the describe-photo route', async () => {
    ...
    await describePhoto('letter-1', 'Likely Jimmy and Molly');
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: 'POST',
        credentials: 'include',
        body: JSON.stringify({ photoDescriptionContext: 'Likely Jimmy and Molly' }),
      }),
    );
  });
  ...
  it('sends photo description updates using the backend route contract', async () => {
    ...
    await updatePhotoDescription('letter-1', 'Two children standing on a porch.');
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: 'PUT',
        credentials: 'include',
        body: JSON.stringify({ photoDescription: 'Two children standing on a porch.' }),
      }),
    );
  });
});

...
export function journalImageSourcesInPost(markdown: string, hero: string): string[] {
  ...
  function walk(node: Root | Root['children'][number]) {
    if (node.type === 'definition') definitions.set((node as Definition).identifier.toLowerCase(), (node as Definition).url);
  ...
  walk(tree);
  return [...new Set([hero.trim(), ...images.map(node => node.type === 'image' ? node.url : definitions.get(node.identifier.toLowerCase()) ?? '')])].filter(source => source.length > 0 && source.length <= 2048).slice(0, 64);
}
